...
describe("worktree harness (ADR 0011)", () => {
  it("creates a worktree of HEAD containing only tracked files (no gitignored secrets)", () => {
    const repo = gitRepo();
  ...
  it("removeWorktree tears the worktree down and is idempotent", () => {
    const repo = gitRepo();
    const wt = createWorktree(repo);
    removeWorktree(wt.path);
    expect(existsSync(wt.path)).toBe(false);
    // Second remove (already gone) must not throw.
    expect(() => removeWorktree(wt.path)).not.toThrow();
    // Removing a never-created path must not throw.
    expect(() => removeWorktree(join(tmpdir(), "houge-wt-does-not-exist"))).not.toThrow();
  });

  it("throws when the project root is not a git repository", () => {
    const notARepo = mkdtempSync(join(tmpdir(), "houge-wt-norepo-"));
    repos.push(notARepo);
    expect(() => createWorktree(notARepo)).toThrow();
  });
});
