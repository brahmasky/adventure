...
describe("defaultSelfWriteDeps diff readers — net-new files are visible to guard + reviewer", () => {
  it("an untracked NEW file appears as a status-A raw entry AND with full content in the unified diff", async () => {
    ...
    const raw = await deps.rawDiff(wt);
    expect(raw).toMatch(/A\tsrc\/prompt\/user-time-zone\.ts/);

    const unified = await deps.unifiedDiff(wt);
  ...
  it("SECURITY: an untracked NEW file at a protected path is DENIED by the guard (creation, not just edits)", async () => {
    const wt = worktreeFixture();
    mkdirSync(join(wt, "src", "policy"), { recursive: true });
    writeFileSync(join(wt, "src", "policy", "evil.ts"), "export const allowEverything = true;\n");
    const deps = defaultSelfWriteDeps();

    const result = checkSelfWriteDiff(parseDiffRaw(await deps.rawDiff(wt)));
  ...
  it("an untracked NEW file at an allowed path passes the guard and reaches the reviewer diff", async () => {
    const wt = worktreeFixture();
    writeFileSync(join(wt, "src", "prompt", "foo.ts"), "export const foo = 42;\n");
    const deps = defaultSelfWriteDeps();

    expect(checkSelfWriteDiff(parseDiffRaw(await deps.rawDiff(wt)))).toEqual({ allowed: true });
    const unified = await deps.unifiedDiff(wt);
    expect(unified).toContain("src/prompt/foo.ts");
    expect(unified).toContain("foo = 42");
  });

  it("the gitignored node_modules symlink is NEVER registered by the intent-to-add step", async () => {
    const wt = worktreeFixture();
    ...
    expect(await deps.unifiedDiff(wt)).not.toContain("node_modules");
  });

  it("refine-loop re-checks stay correct: a file added AFTER the first diff shows on the next one (idempotent add -N)", async () => {
    const wt = worktreeFixture();
    writeFileSync(join(wt, "src", "prompt", "first.ts"), "export const a = 1;\n");
    const deps = defaultSelfWriteDeps();
    expect(await deps.rawDiff(wt)).toMatch(/A\tsrc\/prompt\/first\.ts/);
    ...
    const raw2 = await deps.rawDiff(wt);
    expect(raw2).toMatch(/A\tsrc\/prompt\/first\.ts/);
    expect(raw2).toMatch(/A\tsrc\/prompt\/second\.ts/);
    expect(await deps.unifiedDiff(wt)).toContain("const b = 2");
  });
});
