...
- `/radar` — top active idea cards from the daily Idea Radar tick (ADR 0026): code-owned sources (HN/HF/Devpost/GitHub/lobste.rs) → one DATA-framed extract call → bounded `ideas` store with momentum = distinct items × sources. Pre-arm gate: `houge radar --dry-run` (real fetches, zero writes).
