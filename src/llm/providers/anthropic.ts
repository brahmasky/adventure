...
export function createAnthropicProvider(config: LlmAnswerConfig = {}): LlmProvider {
  return {
    name: "anthropic",
    async answer(req: LlmRequest): Promise<LlmResult> {
      ...
      try {
        ...
        return { ok: false, provider: "anthropic", error: `LLM request failed: ${message}` };
      }
      ...
      if (!response.ok) {
        return {
          ok: false,
          provider: "anthropic",
          error: `LLM request returned HTTP ${response.status}`
        };
      }
      ...
      if (answer === undefined) {
        return { ok: false, provider: "anthropic", error: "LLM response missing text content" };
      }

      return { ok: true, provider: "anthropic", model, answer };
    }
  };
}
